...
class ListViewTest2 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<String> personsList = ['김유신', '김춘추', '장보고', '강감찬', '이순신',
      '정약용', '안중근', '유관순', '안창호', '김구'];
    ...
    return MaterialApp(
      home: Scaffold(
        ...
        body: ListView.builder(           // ListView.builder는 아이템 갯수가 많거나 동적으로 위젯을 생성해서 메모리 자원을 효율적으로 사용, Lazy Loading으로 성능 극대화
         itemCount: personsList.length,   // 목록에 출력할 총 아이템 갯수
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      home: Scaffold(
        ...
        body: ListView.separated(           // ListView.seperated는 ListView.builder에서 목록 구분선을 커스텀 할 수 있는 위젯
